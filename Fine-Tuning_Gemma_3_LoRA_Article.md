...
- **Running the Code** (on CPU): ~2-4 hours (faster with GPU if available)
- **Total**: ~3-5 hours for complete mastery on CPU
...
2. Hardware: **CPU works fine** for this tutorial with Gemma 3 270M. GPU is optional but speeds up training.
...
|---------|------|------|----------|
| **JAX** | Fastest training, XLA compilation, works on CPU and GPU | Newer ecosystem | Production training |
...
- Best performance for LoRA fine-tuning on CPU
- Efficient XLA compilation works on both CPU and GPU
...
- Works well on CPU with reasonable memory usage
...
**Setting**: Should match your data length!

This limits how much text the model processes at once. Think of it as the model's "attention span."

- **Match your data length**: If your Q&A pairs are ~512 tokens, use `512`. If they're longer, use a longer sequence length.
- **Don't go too short**: Model can't see enough context
- **Don't go too long unnecessarily**: Wastes memory and slows down training

```python
gemma_lm.preprocessor.sequence_length = 512  # Adjust based on your data
```
...
**Why 1?** Memory constraints on CPU. Larger batches need more RAM. If you have more memory available, you can try larger batch sizes.

### Training Time Expectations

- **CPU (What we're using)**: ~40-90 minutes per epoch, 2-4 hours total for 3 epochs
- **GPU (Optional)**: ~15-30 minutes total if you have one available

### What to Watch During Training

- **Loss**: Should decrease across epochs (lower is better!)
- **Accuracy**: Should increase across epochs (higher is better!)
- **Memory**: Monitor your system memory usage to ensure you don't run out
...
- **If you have more RAM available**: Try `batch_size=2` or `batch_size=4`
- **Impact**: Faster training, more stable gradient updates
...
- **Current**: Match your data length
- **Try**: Longer sequence length if your data requires it (needs more memory)
...
**For Faster Training (If you have more memory or GPU)**:
1. ✅ Increase batch_size to 2 or 4
...
2. **Monitor memory**: Check system memory usage to ensure you don't run out
...
📓 **Notebook**: This notebook is also available on our GitHub: [ElHadheqMind/Fine-Tuning-Gemma-3](https://github.com/ElHadheqMind/Fine-Tuning-Gemma-3)

👉 **Follow us for more**: [github.com/ElHadheqMind](https://github.com/ElHadheqMind)

🌐 **Our Website**: [elhadheqmind.com](https://elhadheqmind.com/)
